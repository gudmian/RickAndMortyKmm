...
final class LocationFeedViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {

    private let viewModel = FeatureFactoryKt.locationFeedViewModel()
    private var state: LocationFeedViewModel.State?
    ...
    private func apply(_ state: LocationFeedViewModel.State) {
        self.state = state
    ...
    func collectionView(_ cv: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let item = state?.items[indexPath.item] {
            cell.configure(
                title: item.name,
                subtitle: item.type.isEmpty ? "Unknown type" : item.type,
                detail: item.dimension.isEmpty ? "Unknown dimension" : item.dimension,
                imageURL: nil
            )
        }
        return cell
    ...
    func collectionView(_ cv: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        navigationController?.pushViewController(LocationDetailViewController(id: id), animated: true)
    }
}
