...
final class EpisodeFeedViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {

    private let viewModel = FeatureFactoryKt.episodeFeedViewModel()
    private var state: EpisodeFeedViewModel.State?
    ...
    private func apply(_ state: EpisodeFeedViewModel.State) {
        self.state = state
    ...
    func collectionView(_ cv: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let item = state?.items[indexPath.item] {
            cell.configure(
                ...
                detail: item.airDate.isEmpty ? "Unknown air date" : item.airDate,
                imageURL: nil
            )
        }
        return cell
    ...
    func collectionView(_ cv: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        navigationController?.pushViewController(EpisodeDetailViewController(id: id), animated: true)
    }
}
