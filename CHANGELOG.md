...
### Сделано
- Удалён старый модуль `:sharedLogic`. Добавлены `:core` и `:shared` (KMP, фреймворк `SharedKit`).
...
- **KMP-библиотеки** (`core`, `*Api`, `*Impl`, `shared`) используют `kotlinMultiplatform` + `androidMultiplatformLibrary` (как прежний `sharedLogic`), таргеты `androidLibrary` + `iosArm64` + `iosSimulatorArm64`.
...
### Риски
- iOS-проект Xcode ещё ссылается на `SharedLogic` и таску `:sharedLogic:embedAndSignAppleFrameworkForXcode` — будет исправлено в P8 (переход на UIKit + `SharedKit`).
- Не под git — коммиты не выполняются.
