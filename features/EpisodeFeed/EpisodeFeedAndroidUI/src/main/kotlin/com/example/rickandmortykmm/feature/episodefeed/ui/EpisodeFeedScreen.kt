...
@Composable
fun EpisodeFeedScreen(
    onEpisodeClick: (Int) -> Unit,
    viewModel: EpisodeFeedViewModel = koinViewModel(),
) {
    ...
    when {
        ...
        state.error != null && state.items.isEmpty() -> ErrorBox(state.error!!) { viewModel.retry() }
            ...
            items(state.items, key = { it.id }) { item ->
                EpisodeFeedCard(item) { onEpisodeClick(item.id) }
            }
...
@Composable
private fun EpisodeFeedCard(item: EpisodeFeedItem, onClick: () -> Unit) {
    Surface(
        ...
        Column(Modifier.padding(14.dp)) {
            ...
            Text(
                item.airDate.ifBlank { "Unknown air date" },
